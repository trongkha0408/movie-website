...
$result;
//  khi bắt được sự kiện của nút submit
if (isset($_POST['submit'])) {
    //Kiểm tra các input có nhập đầy đủ chưa
    if ($_POST["Images"] != "" && $_POST["Names"] != "" && $_POST["Descr"] != "" && $_POST["Actor"] != "" && $_POST["Director"] != "" && $_POST["Trailer"] != "" && $_POST["Age"] != "" && $_POST["Rating"] != "" && $_POST["year"] != "") {
        ...
        $Rating = $_POST['Rating'];
        $year = $_POST['year'];
        if ($Rating = $_POST['Rating'] > 10) {
            $result = "<div style='color: red;'>Rating phải nhỏ hơn hoặc bằng 10</div>";
        } elseif ($year > date('Y')) {
            $result = "<div style='color: red;'>Năm sản xuất không chính xác</div>";
        } else {
            //truy vấn sql
